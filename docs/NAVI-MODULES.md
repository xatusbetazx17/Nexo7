...
**Nexo-Mobile** is a standalone installable web app, not a signed Android/iOS store binary. Serve the release ZIP’s files from a trusted HTTPS static host, or build with `cd mobile && npm ci --ignore-scripts && npm run build`. The host must serve `.wasm` as `application/wasm`. Open on the phone, choose Install / Add to Home Screen, and import the encrypted `.nexo` file. Current browsers with WebCrypto Ed25519 and WebAssembly SIMD are required. The default engine uses WebAssembly Memory64; a self-hosted compatibility build is included for Safari. Safari/iOS compatibility depends on browser version; physical-device performance has not been benchmarked.

Mobile can create a fresh identity, chat with an optional pinned LiquidAI LFM2.5-350M Q4_K_M model (~230 MB download plus ~24 MB app/runtime), use saved notes as bounded reference, change personality, save notes, and deliver reminders while open. All inference uses llama.cpp through Wllama 3.6.1 CPU WASM on the phone; no desktop server/account/cloud inference is required. This small multilingual model is limited and can be wrong; it is not a replacement for larger desktop models on complex or knowledge-intensive tasks. The context is fixed at 2,048 tokens, input text is conservatively bounded to 1,600 UTF-8 bytes, and replies are capped at 128 tokens. Older reference/history messages are removed first; oversized questions ask you to shorten them. The [LFM Open License](https://huggingface.co/LiquidAI/LFM2.5-350M/blob/main/LICENSE) applies to the downloaded model, including its commercial-use revenue threshold; a copy is included in the mobile app. Model downloads contact Hugging Face only when loading; cached app/model files support offline reuse; public model files are stored in bounded IndexedDB chunks, and model size/SHA-256 are verified before loading. Use Free model memory to stop/unload. Browser storage is managed by the OS and can be evicted.
...
The mobile validation run used actual CPU inference, including offline reload and encrypted desktop/browser round trips. A short English/Spanish diagnostic sampled about 1.63 GiB peak proportional memory across the test process and Chromium; this is not a hard memory cap or a physical phone benchmark. See [the validation record](validation/navi-mobile.json). The compatibility-runtime gate uses a Safari user agent in Chromium to select the fallback engine; it does not certify Safari itself.
