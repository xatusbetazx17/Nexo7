...
If an older version repeatedly waited about 180 seconds, install v0.17.0 in a new folder after quitting Nexo. Open the new executable and start a New chat. Existing downloaded models and saved data remain in the usual per-user data directory. This update reduces prompt and output work but cannot guarantee a specific speed on an untested CPU.
